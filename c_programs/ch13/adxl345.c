...
/* Buffer to store multiple bytes of data from ADXL345 */
...
void adxl_write(uint8_t reg, char value){
    char data[1];
    data[0] = value;
    i2c1_byte_burst_write(ADXL345_DEVICE_ADDR, reg, 1, data);
}

void adxl_read_values(uint8_t reg){
    /* Read 6 bytes into the data buffer */
    i2c1_byte_burst_read(ADXL345_DEVICE_ADDR, reg, 6, (char *)data_buffer);
}
...
void adxl_init(void){
    ...
    /* Read DEVID, this should return 0xE5 */
    adxl_read_address(ADXL345_REG_DEVID);

    /* Set dat format range to +-4g */
