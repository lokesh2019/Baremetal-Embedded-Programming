...
/* PA0 is the wake up (WKUP) pin. Rising edge on this pin wakes up the board. */
...
void standby_wakeup_pin_setup(void){
    ...
    /* Disable wakeup pin so that any fluctuations there don't interrupt
    ...
    /* All good. Now we can enable wakeup pin */
    PWR->CSR |=(1U<<8);
...
/* This is an active low pin. If pin's value is 0, the pin is Active */
